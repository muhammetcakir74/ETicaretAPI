...
Logger log = new LoggerConfiguration()
    ...
    .WriteTo.Seq(builder.Configuration["Seq:ServerURL"])//Seq te görsel log sistemi
    .Enrich.FromLogContext() //Dýþ columwriter kullanýmý için (bizdeki UserNameColumnWriter)
    .MinimumLevel.Information()//Hangi durumlarda log tutulacaðý error yaparsak sadece errorda log kaydý yapar.
    .CreateLogger();
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer("Admin",options =>
{
    options.TokenValidationParameters = new()
    {
        ...
        //Token'ýn süresinin doðru hesaplanmasý için.
        LifetimeValidator = (notBefore, expires, securityToken, validationParameters) => expires != null ? expires > DateTime.UtcNow : false,
    
        NameClaimType = ClaimTypes.Name //JWT üzerinde Name claime karþýlýk gelen deðeri User.Identity.Name propertisinden elde edebiliriz.
    };
});
